...
def YF(k, SIG, yfun, keep=None, dofort=True):
    ...
    if dofort:
        if 'YLD2004_09p' in model and len(coef) == 4:
            # C12, C13, C31, C44
            coef_mod = np.zeros(9)
            coef_mod[[0,1,4,6]] = coef
        ...
        elif 'YLD2004_09p' in model and len(coef) == 9:
            f = fyf.yld2004_9p(sig, coef, m)
        
        elif 'YLD2004_18p' in model and len(coef) == 7:
        ...
        elif 'YLD2004_18p' in model and len(coef) == 18:
            f = fyf.yld2004_18p(sig, coef, m)
            
        elif '27p' in model and len(coef) == 12:
            # C12, C13, C31, C44, D12, D13, D31, D44, E12, E13, E31, E44
            coef_mod = np.zeros(27)
            coef_mod[[0,1,4,6,9,10,13,15,18,19,22,24]] = coef
            coef_mod[2] = coef_mod[0]
            coef_mod[3] = coef_mod[1]
            coef_mod[5] = coef_mod[4]
            coef_mod[7] = coef_mod[6]
            ...
            if 'YLD2004_27p' in model:
                f = fyf.yld2004_27p(sig, coef_mod, m)
            elif 'YLD2011_27p' in model:
                f = fyf.yld2011_27p(sig, coef_mod, m)
            elif 'YLD2013_27p' in model:
                f = fyf.yld2013_27p(sig, coef_mod, m)
            
        elif 'YLD2004_27p' in model and len(coef) == 27:
            f = fyf.yld2004_27p(sig, coef, m)
            
        elif 'YLD2011_18p' in model and len(coef) == 18:
            f = fyf.yld2011_18p(sig, coef, m)
            
        elif 'YLD2011_27p' in model and len(coef) == 27:
            f = fyf.yld2011_27p(sig, coef, m)
            
        elif 'YLD2013_27p' in model and len(coef) == 27:
            f = fyf.yld2013_27p(sig, coef, m)
            
        elif 'YLD2011_18p' in model and len(coef) == 8:
            # C12, C13, C31, C44, D12, D13, D31, D44
            coef_mod = np.zeros(18)
            coef_mod[[0,1,4,6,9,10,13,15]] = coef
            coef_mod[2] = coef_mod[0]
            coef_mod[3] = coef_mod[1]
            coef_mod[5] = coef_mod[4]
            coef_mod[7] = coef_mod[6]
        ...
        elif 'YLD2013_Xp' in model and len(coef) == 200:
        ...
        if model == 'YLD2004_27p':    

            C12, C13, C21, C23, C31, C32, C44, C55, C66, D12, D13, D21, D23, D31, D32, D44, D55, D66, E12, E13, E21, E23, E31, E32, E44, E55, E66 = coef
            ...
            # the analysis on coefficient-linear dependency has to be done 
...
#     coef = coefa
    ...
    a = coefa[-1] 
    coef_mod = np.zeros(27)
    if model == 'YLD2004_09p' and len(coef) == 4:
        # C12, C13, C31, C44
        coef_mod[[0,1,4,6]] = coef
        coef_mod[2] = coef_mod[0]
        coef_mod[3] = coef_mod[1]
        ...
        coef_mod[7] = coef_mod[6]
        coef_mod[8] = coef_mod[0]
    elif model == 'YLD2004_18p' and len(coef) == 7:
        # C12, C13, C31, C44, D12, D13, D44
        coef_mod[[0,1,4,6,9,10,15]] = coef
        coef_mod[2] = coef_mod[0]
        coef_mod[3] = coef_mod[1]
        ...
        coef_mod[16] = coef_mod[15]
        coef_mod[17] = coef_mod[9]
    elif model[-3:] == '27p' and len(coef) == 12:
        # C12, C13, C31, C44, D12, D13, D31, D44, E12, E13, E31, E44
        coef_mod[[0,1,4,6,9,10,13,15,18,19,22,24]] = coef
        coef_mod[2] = coef_mod[0]
        coef_mod[3] = coef_mod[1]
        ...
        coef_mod[25] = coef_mod[24]
        coef_mod[26] = coef_mod[18]
    elif model == 'YLD2011_18p' and len(coef) == 8:
        # C12, C13, C31, C44, D12, D13, D31, D44
        coef_mod[[0,1,4,6,9,10,13,15]] = coef
        coef_mod[2] = coef_mod[0]
        coef_mod[3] = coef_mod[1]
...
def genStresses(N, yfun, plot_axes, save2file=False, dofort=True):
    ...
    for s0 in s0_list:
        ...
        for i in range(N):  
            k, infodict, ier, mesg = fsolve(YF, x0=yfun['y0'], args=(s[:,:,i], yfun, abs_comp, dofort), full_output=True)
            if ier == 1:
                stresses[:,:,j] = k*s[:,:,i]
                if zij is not None:
                    stresses[zi,zj,j] = s0
